...
public final class DeltaBlue extends BenchmarkBase {
  ...
  private void chainTest(final int n) {
    ...
    ArrayList<EditConstraint> arg = new ArrayList();
    arg.add(edit);
    Plan plan = planner.extractPlanFromConstraints(arg);
    for (int i = 0; i < 100; i++) {
      first.value = i;
      plan.execute();
      if (last.value != i) {
  ...
  private void projectionTest(final int n) {
    ...
    ArrayList<Variable> dests = new ArrayList(n);
    for (int i = 0; i < n; i++) {
      src = new Variable(i);
      dst = new Variable(i);
      dests.add(dst);
  ...
  private void change(Variable v, int newValue) {
    ...
    ArrayList<EditConstraint> arg = new ArrayList();
    arg.add(edit);
    Plan plan = Planner.INSTANCE.extractPlanFromConstraints(arg);
    for (int i = 0; i < 10; i++) {
      v.value = newValue;
      plan.execute();
    }
